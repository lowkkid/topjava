...
//        mealsTo.forEach(System.out::println);
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesPerDayStat = new HashMap<>();

        for (UserMeal m : meals) {
            LocalDate mealDate = m.getDateTime().toLocalDate();
            caloriesPerDayStat.merge(mealDate, m.getCalories(), Integer::sum);
        }
        ...
        for (UserMeal m: meals) {
            if (TimeUtil.isBetweenHalfOpen(m.getDateTime().toLocalTime(), startTime, endTime)) {
                result.add(createUserMealWithExcess(
                        m,
                        caloriesPerDayStat.get(m.getDateTime().toLocalDate()) > caloriesPerDay));
            }
        }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesPerDayStat = meals
                .stream()
        ...
        return meals
                ...
                .map(meal -> createUserMealWithExcess(meal, caloriesPerDayStat.get(meal.getDate()) > caloriesPerDay))
